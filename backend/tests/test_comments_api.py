...
class TestComments:
    async def test_create_comment(self, client: AsyncClient, owner_user, tender):
        ...
        resp = await client.post("/api/v1/comments", json={
            "tender_id": tender.id, "content": "Test @charlie mention",
    ...
    async def test_list_comments(self, client: AsyncClient, owner_user, tender):
        ...
        await client.post("/api/v1/comments", json={
            "tender_id": tender.id, "content": "First comment",
        }, headers={"Authorization": f"Bearer {token}"})
        resp = await client.get(f"/api/v1/comments?tender_id={tender.id}",
                                headers={"Authorization": f"Bearer {token}"})
