...
class TestRBAC:
    ...
    async def test_employee_cannot_create_agency(self, client: AsyncClient, employee_user):
        ...
        resp = await client.post("/api/v1/agencies", json={"name": "Hacked"}, headers={"Authorization": f"Bearer {token}"})
    ...
    async def test_owner_can_create_agency(self, client: AsyncClient, owner_user):
        ...
        resp = await client.post("/api/v1/agencies", json={"name": "New Agency"}, headers={"Authorization": f"Bearer {token}"})
        assert resp.status_code == 200

    async def test_employee_cannot_create_workspace(self, client: AsyncClient, employee_user):
        token = _token(employee_user.id, "employee", employee_user.agency_id)
        resp = await client.post("/api/v1/workspaces", json={"name": "Hacked"}, headers={"Authorization": f"Bearer {token}"})
        assert resp.status_code == 403

    async def test_manager_can_create_workspace(self, client: AsyncClient, manager_user):
        token = _token(manager_user.id, "manager", manager_user.agency_id)
        resp = await client.post("/api/v1/workspaces", json={"name": "New Workspace"}, headers={"Authorization": f"Bearer {token}"})
        assert resp.status_code == 200
    ...
    async def test_employee_can_view_tenders(self, client: AsyncClient, employee_user, tender):
        token = _token(employee_user.id, "employee", employee_user.agency_id)
    ...
    async def test_employee_can_update_own_task(self, client: AsyncClient, owner_user, employee_user, tender):
        ...
        create_resp = await client.post("/api/v1/tasks", json={
            "tender_id": tender.id, "title": "Employee Task", "assigned_to": employee_user.id,
    ...
    async def test_cross_agency_isolation(self, client: AsyncClient, agency, db_session):
        ...
        other_user = User(id=999, email="other@other.com", password_hash=get_password_hash("test"), role=UserRole.OWNER, agency_id=other_agency_id, full_name="Other")
